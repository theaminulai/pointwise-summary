...
import { getDefaultModelColor } from '../../utils/modelDefaultColors';
...
const PlatformButton: React.FC< PlatformButtonProps > = ( {
	...
	const defaultColor = modelColor ?? getDefaultModelColor( logoKey );
	const customStyle =
		buttonStyle === 'default'
			? {
	...
	return (
		<button
			className={ `cursor-pointer ${ className } ${ padding }` }
			style={ customStyle }
		>
			{ iconDisplay !== 'text-only' && Logo && (
...
const PlatformList: React.FC< PlatformListProps > = ( {
	...
	<div
		className={ ` cursor-pointer bg-white border border-gray-200 rounded-lg p-2 shadow-md space-y-1 min-w-[150px] ${
			className ?? ''
		}` }
	>
...
const InlineButtonGroup: React.FC< InlineButtonGroupProps > = ( {
	...
	return (
		<div
			className={ `flex flex-wrap items-center gap-2 ${ getAlignmentClasses(
				alignment
			) }` }
		>
			{ enabledPlatforms.length > 0 ? (
				...
				<button
					className={ `${ baseButtonClasses } ${
						iconDisplay === 'icons-only' ? 'p-2' : 'px-3 py-2'
					} cursor-pointer` }
				>
					{ iconDisplay !== 'text-only' && (
...
const FloatingButton: React.FC< FloatingButtonProps > = ( {
	...
	const firstEnabledPlatform = enabledPlatforms[ 0 ];
	const defaultColor =
		firstEnabledPlatform?.color ??
		getDefaultModelColor( firstEnabledPlatform?.logoKey ?? 'chatgpt' );
	const askAiStyle =
		buttonStyle === 'default'
			? {
					backgroundColor: defaultColor,
	...
	return (
		<div
			className={ `flex ${ getAlignmentClasses( alignment ) } relative` }
		>
			<div className="relative inline-flex flex-col items-start gap-2">
				{ floating && showList && (
					<PlatformList
						platforms={ enabledPlatforms }
						className={ getFloatingMenuPositionClasses(
							floatingPosition
						) }
					/>
				) }
				...
				{ ! floating && showList && (
					<PlatformList
						platforms={ enabledPlatforms }
						className={ collapsedPositionClass( alignment ) }
					/>
				) }
			</div>
		</div>
...
export const PreviewButton: React.FC< PreviewButtonProps > = ( {
	...
	const baseButtonClasses = [
		...
		.filter( Boolean )
		.join( ' ' );
	const showPerModelButtons =
		displayMode === 'inline' || ( displayMode === 'both' && ! floating );
