...
import { Bookmark, Facebook, Linkedin, Mail, Send, Twitter } from 'lucide-react';
import type * as React from 'react';
import { useCallback, useMemo } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { useSocialSharingUpdate } from '../../hooks/useSocialSharingUpdate';
import { toggleNetwork, updateNetworkMention } from '../../store/socialSharing.actions';
...
export const SocialNetworks: React.FC = () => {
	...
	const networksWithIcons = useMemo<NetworkWithIcon[]>(
		() => networks.map((network) => ({ ...network, icon: ICON_MAP[network.iconKey] })),
		[networks]
	);
	...
	return (
		<div className="bg-white rounded-lg shadow-sm border border-gray-200">
			<div className="p-6 border-b border-gray-200">
				<h3 className="text-gray-900">{__('Social Networks', 'pointwise-summary')}</h3>
				<p className="text-sm text-gray-600 mt-1">
					{sprintf(
						__('%1$d of %2$d networks enabled', 'pointwise-summary'),
						enabledCount,
						networks.length
					)}
				</p>
